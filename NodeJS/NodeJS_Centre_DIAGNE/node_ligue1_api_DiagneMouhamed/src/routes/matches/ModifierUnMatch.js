...
module.exports = (app) => {
  app.put("/api/matchs/:id", (req, res) => {
    ...
    // Si les équipes ont changé, on doit recalculer l'id
    const nouvelleEquipeRecevante = matchModifiee.idEquipeRecevante;
    const nouvelleEquipeDeplacée = matchModifiee.idEquipeDeplacée;
    const nouvelId = `${nouvelleEquipeRecevante}-${nouvelleEquipeDeplacée}`;

    // Si l'id change, vérifier qu'il n'existe pas déjà ailleurs
    if (nouvelId !== idMatchDemande) {
      const doublon = matchs.some(
        (m, i) => m.id === nouvelId && i !== index
      );
      if (doublon) {
        return res.status(400).json({
          message: "Un match avec ce couple d'équipes existe déjà !",
        });
      }
      matchModifiee.id = nouvelId;
    }
