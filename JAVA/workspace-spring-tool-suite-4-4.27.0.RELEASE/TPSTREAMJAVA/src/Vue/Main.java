	...
	public static void main(String[] args) {
		...
		try {
			stream = Files.lines(Paths.get(
					"/Users/utilisateur/Documents/workspace-spring-tool-suite-4-4.27.0.RELEASE/TPSTREAMJAVA/bin/Entity/films.txt"));

			map = stream.map(t -> t.split(";"))
			...
			Stream<String> Stacteur1 = map.values().stream()
					.flatMap(List::stream);
			// regrouper les acteur et compter leurs apparitions
			Map<String, Long> acteurCount = Stacteur1
					.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
			// afficher acteur et son nombre son film
			acteurCount.entrySet().stream().limit(5).forEach(entry -> System.out
					.println("Acteur :" + entry.getKey() + " | nombre de films " + entry.getValue()));
